...
import eu.slickbot.conver.domain.converter.padLeft
import eu.slickbot.conver.domain.converter.padRight
...
fun paperSizeConverter(): TextConverter = TextConverter(
  ...
  name = "Paper sizes",
  category = Category.Everyday,
  icon = ConverterIcon.Description,
  ...
  modes = listOf(
    TextConverter.Mode("lookup", "Look up") { input ->
      ...
      "${match.widthMm} × ${match.heightMm} mm\n${fixed(match.widthIn, 1)} × ${fixed(match.heightIn, 1)} in"
    },
    TextConverter.Mode("all", "List all", inputless = true) { _ ->
      paperSizes.joinToString("\n") { p ->
        padRight(p.name, 10) + " " + padLeft(p.widthMm, 4) + " × " + padLeft(p.heightMm, 4) + " mm"
      }
    },
  ),
)
...
fun ovenTempConverter(): MeasurementConverter = MeasurementConverter(
  ...
  category = Category.Everyday,
  icon = ConverterIcon.LocalFireDepartment,
...
fun bmiConverter(): CalculatorConverter = CalculatorConverter(
  ...
  name = "BMI calculator",
  category = Category.Everyday,
  icon = ConverterIcon.FitnessCenter,
  ...
  calculate = { modeId, inputs ->
    ...
    listOf(
      CalculatorConverter.Result("BMI", fixed(bmi, 1)),
...
fun runningPaceConverter(): MeasurementConverter = MeasurementConverter(
  ...
  name = "Running pace",
  category = Category.Everyday,
  icon = ConverterIcon.DirectionsRun,
  ...
  modes = listOf(
    TextConverter.Mode("calc", "Calculate") { input ->
      ...
      planets.joinToString("\n") { (name, orbitalPeriod) ->
        padRight(name, 10) + " " + fixed(earthYears / orbitalPeriod, 2) + " years"
      }
    },
  ),
)
