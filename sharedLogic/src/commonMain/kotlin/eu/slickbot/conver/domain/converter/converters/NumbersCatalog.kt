...
import eu.slickbot.conver.domain.converter.hex4
...
private fun codePointAt0(s: String): Int {
  val first = s[0]
  if (first.isHighSurrogate() && s.length > 1 && s[1].isLowSurrogate()) {
    return 0x10000 + ((first.code - 0xD800) shl 10) + (s[1].code - 0xDC00)
  }
  return first.code
}
...
fun unicodeConverter(): TextConverter = TextConverter(
  ...
  modes = listOf(
    TextConverter.Mode("char-to-cp", "Char → Code point") { input ->
      ...
      val cp = codePointAt0(s)
      "U+" + hex4(cp) + " · decimal " + cp
    },
